...
const TETROMINO_PIVOTS = {
  I: [1.5, 0.5], J: [1, 1], L: [1, 1], O: [0.5, 0.5], S: [1, 1], T: [1, 1], Z: [1, 1],
};
...
const checkCollision = (cellsToCheck, currentTetromino, placedTetrominos) => {
  for (const [x, y] of cellsToCheck) {
    ...
    for (const placed of placedTetrominos) {
      ...
      if (placed.absolutePositions.some(([px, py]) => x === px && y === py)) return true; // 重なりあり
    }
  }
  return false; // 衝突なし
};
...
export default function DeckMain() {
  ...
  const [selectedTetrominoType, setSelectedTetrominoType] = useState<string | null>(null);
  const [potentialScore, setPotentialScore] = useState(0);
  const [draggedTetrominoId, setDraggedTetrominoId] = useState<number | null>(null);
  ...
  const transformContributionsToGrid = (contributions) => {
      const grid = Array(GRID_HEIGHT).fill(0).map(() => Array(GRID_WIDTH).fill({ count: 0, level: 0 }));
      // 56日分のデータをグリッドにマッピング
  ...
  const calculateTetrominoDetails = useCallback((type, currentX, currentY, currentRotation) => {
    if (!type) return { absolutePositions: [], scorePotential: 0, blockDetails: [] };
    const relativeShape = rotateShape(TETROMINO_SHAPES[type], TETROMINO_PIVOTS[type], currentRotation);
    const absolutePositions = relativeShape.map(([rx, ry]) => [currentX + rx, currentY + ry]);
    ...
    absolutePositions.forEach(([ax, ay]) => {
      let level = 0;
      if (ay >= 0 && ay < GRID_HEIGHT && ax >= 0 && ax < GRID_WIDTH && contributionGrid.length > 0 && contributionGrid[ay] && contributionGrid[ay][ax]) {
  ...
  useEffect(() => {
    const newScore = placedTetrominos.reduce((sum, piece) => sum + (piece.scorePotential || 0), 0);
  ...
  useEffect(() => {
    if (!loading && contributionGrid.length > 0 && placedTetrominos.length > 0) {
      setPlacedTetrominos(currentTetrominos =>
        currentTetrominos.map(t => {
          const { scorePotential, blockDetails, absolutePositions } = calculateTetrominoDetails(
            t.type,
            t.x,
            t.y,
            t.rotation
  ...
  const getAdjustedDropPosition = useCallback((type, rotation, dropX, dropY) => {
    const relativeShape = rotateShape(TETROMINO_SHAPES[type], TETROMINO_PIVOTS[type], rotation);
  ...
  const handlePlacedTetrominoDragStart = (e, id) => {
    ...
    setSelectedTetrominoType(placedTetrominos.find(t => t.id === id)?.type || null);
  ...
  const handleDrop = useCallback((e) => {
    ...
    if (!isFinite(dropGridX) || !isFinite(dropGridY)) {
        console.error("Calculated drop coordinates are not finite.", { dropGridX, dropGridY });
        setMessage("ドロップ位置の計算に失敗しました。");
        return;
    }

    const handlePlacement = (type, rotation, baseDropX, baseDropY, id = null) => {
      ...
      const { adjustedX, adjustedY } = getAdjustedDropPosition(type, rotation, centeredDropX, centeredDropY);
      
      const details = calculateTetrominoDetails(type, adjustedX, adjustedY, rotation);

      if (checkCollision(details.absolutePositions, { id, absolutePositions: details.absolutePositions }, placedTetrominos)) {
        setMessage('その場所には配置できません。');
        return null;
      }
      return { id: id || Date.now(), type, x: adjustedX, y: adjustedY, rotation, ...details };
    };

    if (isDraggingNew && selectedTetrominoType) {
      const newPiece = handlePlacement(selectedTetrominoType, 0, dropGridX, dropGridY);
      if (newPiece) {
  ...
  }, [isDraggingNew, selectedTetrominoType, placedTetrominos, draggedTetrominoId, calculateTetrominoDetails, getAdjustedDropPosition]);
  ...
  const handleRotate = useCallback((id) => {
    setPlacedTetrominos(prev => {
      ...
      const kickTranslations = [[0, 0], [-1, 0], [1, 0], [-2, 0], [2, 0]]; // [dx, dy]
      ...
      for (const [dx, dy] of kickTranslations) {
        ...
        const { adjustedX, adjustedY } = getAdjustedDropPosition(type, newRotation, testX, testY);

        const details = calculateTetrominoDetails(type, adjustedX, adjustedY, newRotation);
        const tempTetromino = { ...tetromino, absolutePositions: details.absolutePositions };
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center p-4" style={{ backgroundColor: UI_COLORS.darkBackground, fontFamily: '"DotGothic16", monospace' }}>
      <style>{`@import url('https://fonts.googleapis.com/css2?family=DotGothic16&display=swap');`}</style>
      <h1 className="text-4xl font-bold mb-8" style={{ color: UI_COLORS.lightText }}>GitHub 草テトリス：デッキ編成</h1>
      ...
      <div className="bg-gray-800 p-6 rounded-xl shadow-lg mb-8 w-full max-w-4xl flex justify-between items-start flex-wrap">
        ...
        <div className="w-full md:w-1/4 mb-6 md:mb-0">
          <h2 className="text-2xl font-semibold mb-4" style={{ color: UI_COLORS.lightText }}>テトリミノパレット</h2>
          <div className="grid grid-cols-2 gap-4">
            {Object.keys(TETROMINO_SHAPES).map(type => {
              const isPlaced = placedTetrominos.some(p => p.type === type);
              return (
                <div key={type} className={`p-3 bg-gray-700 rounded-lg flex flex-col items-center justify-center transition-all ${isPlaced ? 'opacity-40 cursor-not-allowed' : 'cursor-grab hover:scale-105'}`} draggable={!isPlaced} onDragStart={(e) => handlePaletteDragStart(e, type)}>
                  <div className="grid gap-px" style={{ gridTemplateColumns: `repeat(4, 12px)` }}>
                    {Array.from({ length: 4 }).map((_, r) => Array.from({ length: 4 }).map((__, c) => (
                      <div key={`${type}-${r}-${c}`} className="w-3 h-3 rounded-sm" style={{ backgroundColor: 'transparent', border: TETROMINO_SHAPES[type as keyof typeof TETROMINO_SHAPES].some(([x, y]) => x === c && y === r) ? `1px solid ${TETROMINO_COLORS[type as keyof typeof TETROMINO_COLORS][0]}` : 'none' }}></div>
                    )))}
                  </div>
                  <span className="text-sm mt-2 font-bold" style={{ color: UI_COLORS.lightText }}>{type}-ミノ</span>
                </div>
              );
            })}
          </div>
        ...
        <div className="w-full md:w-3/4 flex flex-col items-center">
          <div className="w-full flex justify-between items-center mb-4">
            <h2 className="text-2xl font-semibold" style={{ color: UI_COLORS.lightText }}>GitHub草グリッド (8x7)</h2>
            <button onClick={fetchContributions} disabled={loading} className="bg-blue-600 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded-lg disabled:bg-gray-500">
              {loading ? '取得中...' : 'データ再取得'}
            </button>
          </div>

          {/* メッセージボックス */}
          {message && <div className="bg-blue-500 text-white px-4 py-2 rounded-lg mb-4 text-center">{message}</div>}
          {error && <div className="bg-red-500 text-white px-4 py-2 rounded-lg mb-4 text-center">エラー: {error}</div>}
          
          {/* ポテンシャルスコア */}
          <div className="text-4xl font-bold mb-6 p-4 rounded-xl" style={{ backgroundColor: UI_COLORS.darkGreen, color: UI_COLORS.lightText, boxShadow: '0 0 15px rgba(86, 211, 100, 0.5)' }}>
            ポテンシャルスコア: <span style={{ color: UI_COLORS.brightGreen }}>{potentialScore}</span>
          </div>
          <div className="mb-4">
            <SaveDeckButton
              tetrominosToSave={placedTetrominos.map(t => {
                const startDate = contributionGrid[t.y]?.[t.x]?.date || new Date().toISOString().split('T')[0];
                const positions = t.blockDetails.map((block: any) => ({ x: block.x, y: block.y, score: block.score }));
                const payload: TetrominoPlacementPayload = { type: t.type, rotation: t.rotation, startDate, positions, scorePotential: t.scorePotential };
                return payload;
              })}
          ...
          {/* メイングリッド */}
          <div ref={gridRef} className="relative grid rounded-lg shadow-inner bg-black" style={{ gridTemplateColumns: `repeat(${GRID_WIDTH}, ${CELL_PX_SIZE}px)`, gridTemplateRows: `repeat(${GRID_HEIGHT}, ${CELL_PX_SIZE}px)` }} onDrop={handleDrop} onDragOver={handleDragOver}>
            {loading ? ( <div className="absolute inset-0 flex items-center justify-center bg-opacity-50 bg-gray-900 z-50"><p className="text-white text-2xl">Loading...</p></div> ) : (
              contributionGrid.length > 0 && Array.from({ length: GRID_HEIGHT }).map((_, y) => (
                Array.from({ length: GRID_WIDTH }).map((__, x) => {
                  const level = contributionGrid[y]?.[x]?.level ?? 0;
                  return ( <div key={`${x}-${y}`} className="w-9 h-9 rounded-sm box-border" style={{ backgroundColor: getGrassColorFromLevel(level), border: `1px solid ${UI_COLORS.darkBackground}` }}/> );
                })
              ))
            )}
            ...
            {placedTetrominos.map(tetrimino => {
              const pos = tetrimino?.absolutePositions; if (!pos || pos.length === 0) return null;
              const xs = pos.map((p: number[]) => p[0]); const ys = pos.map((p: number[]) => p[1]);
              if (xs.some(v => !isFinite(v)) || ys.some(v => !isFinite(v))) return null;
              
              // テトリミノのバウンディングボックスの最小/最大座標を計算
              const minX = Math.min(...xs); const minY = Math.min(...ys);
              const widthPx = (Math.max(...xs) - minX + 1) * CELL_PX_SIZE; const heightPx = (Math.max(...ys) - minY + 1) * CELL_PX_SIZE;
              const leftPx = minX * CELL_PX_SIZE; const topPx = minY * CELL_PX_SIZE;
              if (!isFinite(leftPx) || !isFinite(topPx) || widthPx < 0 || heightPx < 0) return null;
              return (
                <div key={tetrimino.id} draggable onDragStart={(e) => handlePlacedTetrominoDragStart(e, tetrimino.id)} className="absolute cursor-grab rounded-md group" style={{ left: `${leftPx}px`, top: `${topPx}px`, width: `${widthPx}px`, height: `${heightPx}px`, zIndex: 20 }}>
                  
                  {/* 各テトリミノブロックの描画 */}
                  {tetrimino.blockDetails.map((block: any) => {
                    const blockLeft = (block.x - minX) * CELL_PX_SIZE; const blockTop = (block.y - minY) * CELL_PX_SIZE;
                    if (!isFinite(blockLeft) || !isFinite(blockTop)) return null;
                    return ( <div key={`${tetrimino.id}-${block.x}-${block.y}`} className="absolute w-9 h-9 rounded-sm box-border" style={{ left: `${blockLeft}px`, top: `${blockTop}px`, backgroundColor: TETROMINO_COLORS[tetrimino.type as keyof typeof TETROMINO_COLORS][block.colorIndex] ?? UI_COLORS.darkBackground, border: '1px solid rgba(255,255,255,0.2)' }} /> );
                  })}
                  ...
                  <div className="absolute inset-0 flex items-center justify-center space-x-1 opacity-0 group-hover:opacity-100 transition-opacity">
                    <button onClick={() => handleRotate(tetrimino.id)} className="bg-blue-600 text-white p-1 rounded-full shadow-md hover:bg-blue-700" title="回転"><svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004 12c0 2.21.894 4.204 2.343 5.657M18 19v-5h-.582m0 0a8.001 8.001 0 01-15.356-2m15.356 2c-.065.311-.083.63-.058.95L18 19z"></path></svg></button>
                    <button onClick={() => handleDelete(tetrimino.id)} className="bg-red-600 text-white p-1 rounded-full shadow-md hover:bg-red-700" title="削除"><svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"></path></svg></button>
                  </div>
                </div>
              );
            })}
